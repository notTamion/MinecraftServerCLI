        ...
        try {
            switch (project) {
                case "fabric":
                    ...
                    break;
                case "magma":
                    System.out.println(new ObjectMapper().readTree(new URL("https://api.magmafoundation.org/api/v2/allVersions")).toString().replaceAll("\\[", "").replaceAll("]", "").replaceAll("\"", "").replaceAll(",", ", "));
                    break;
                case "purpur":
