        ...
        try {
            ...
            if(new File(directory + "/mcscli.properties").exists()) {
                ...
                if (props.getProperty("AutoUpdater").equals("true")) {
                    String latestbuild;
                    switch (project) {
                        case "fabric": JsonNode json = new ObjectMapper().readTree(new URL("https://meta.fabricmc.net/v2/versions/loader")); latestbuild = json.iterator().next().get("version").asText(); json = new ObjectMapper().readTree(new URL("https://meta.fabricmc.net/v2/versions/installer"));latestbuild = latestbuild + ":" + json.iterator().next().get("version").asText();break;
                        case "magma": latestbuild = new ObjectMapper().readTree(new URL("https://api.magmafoundation.org/api/v2/" + version + "/latest")).get("name").asText(); break;
                    ...
                    if (!latestbuild.equals(currentbuild)) {
                        ...
                        switch (project) {
                            case "fabric": String[] builds = latestbuild.split(":"); FileUtils.copyURLToFile(new URL("https://meta.fabricmc.net/v2/versions/loader/" + version + "/" + builds[0] + "/" + builds[1] + "/server/jar"), new File(directory + "/server.jar")); break;
                            case "magma": FileUtils.copyURLToFile(new URL("https://api.magmafoundation.org/api/v2/" + version + "/latest/" + latestbuild + "/download"), new File(directory + "/server.jar")); break;
